...
#define STACK 0xFFFF
    ...
    public:
        ...
        void* get_syscall(int index) {
            return syscall_table[index];
        }
        registers* get_context(uint32_t thread_id) {
            return &context[thread_id];
        }
    protected:
        std::unordered_map < uint32_t, registers > context;
        std::vector < virtual_machine* > m_machines;
        uint8_t ram[RAM];
    ...
    public:
        ...
        registers* get_current_context() {
            return get_core()->get_context(thread_id);
        }
